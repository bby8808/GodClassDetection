...
public class DistanceMatrix {
    ...
    public Set<String> getClassMap(String clazz){
	...
	public double[][] getJaccardDistanceMatrix(MyClass[] classes,boolean debug){
		if(classes.length==2)
			return getJaccardDistanceMatrix2(classes[0], classes[1],debug);
		if(classes.length==3)
			return getJaccardDistanceMatrix3(classes[0], classes[1], classes[2], debug);
		return null;
	}
	
	public double[][] getJaccardDistanceMatrix2(MyClass class1, MyClass class2, boolean debug) {
		ArrayList<Entity> entities1 = new ArrayList<Entity>();
		entities1.addAll(class1.getAttributeList());
		entities1.addAll(class2.getAttributeList());
	//	ArrayList<Entity> entities2 = new ArrayList<Entity>();
		entities1.addAll(class1.getMethodList());
		entities1.addAll(class2.getMethodList());
		if(debug){
			printDebugger(class1, entities1);
	//		printDebugger(class2, entities2);
		}

		double[][] jaccardDistanceMatrix = new double[entities1.size()][entities1.size()];
		for(int i=0; i<entities1.size(); i++) {
			for(int j=0; j<entities1.size(); j++) {
				if(i != j) {
					jaccardDistanceMatrix[i][j] = DistanceCalculator.getDistance(entities1.get(i).getFullEntitySet(),
							entities1.get(j).getFullEntitySet());
				}
				else {
					jaccardDistanceMatrix[i][j] = 0.0;
				}
	...
	public double[][] getJaccardDistanceMatrix3(MyClass class1, MyClass class2,
			MyClass class3,boolean dubug) {
		ArrayList<Entity> entities1 = new ArrayList<Entity>();
		entities1.addAll(class1.getAttributeList());
		entities1.addAll(class2.getAttributeList());
		entities1.addAll(class3.getAttributeList());
	//	ArrayList<Entity> entities2 = new ArrayList<Entity>();
		entities1.addAll(class1.getMethodList());
		entities1.addAll(class2.getMethodList());
		entities1.addAll(class3.getMethodList());
//		printDebugger(class1, entities1);
//		printDebugger(class2, entities2);
		double[][] jaccardDistanceMatrix = new double[entities1.size()][entities1.size()];
		for(int i=0; i<entities1.size(); i++) {
	...
	private double[][] getJaccardDistanceMatrix(ArrayList<Entity> entities){
		double[][] jaccardDistanceMatrix = new double[entities.size()][entities.size()];
		for(int i=0; i<jaccardDistanceMatrix.length; i++) {
			for(int j=0; j<jaccardDistanceMatrix.length; j++) {
				if(i != j) {
	...
	public void printDebugger(MyClass clazz, ArrayList<Entity> entities){
//		System.out.println(clazz.getName()+": "+entities.size());
//		System.out.println("\t"+clazz.getAttributeList().size());
//		System.out.println("\t"+clazz.getMethodList().size());
//		for(int i=0; i<clazz.getMethodList().size(); i++){
//			System.out.println("\t"+clazz.getMethodList().get(i).getMethodName());
//		}
		for(Entity entity:entities){
			if(entity instanceof MyAttribute)
				System.out.println(((MyAttribute) entity).getName());
			if(entity instanceof MyMethod)
				System.out.println(((MyMethod) entity).getName());
		}
	}
